...
def test_proto_stage_stripe_checkout_full_success(monkeypatch):
    ...
    assert result.detail["payment_method_id"] == "pm_paypal_99"
    assert result.detail["init_checksum"] == "init-cs-token"
    assert result.detail["paypal_return_url"].startswith("https://pay.openai.com/c/pay/cs_live_demo")

    # 4 次 HTTP，顺序为 init → tax → payment_methods → confirm
    ...
    assert urls == [
        ...
        f"{stripe_http.STRIPE_API_BASE}/payment_methods",
        f"{stripe_http.STRIPE_API_BASE}/payment_pages/cs_live_demo/confirm",
    ]
    # /confirm 体里 init_checksum 和 PaymentMethod 必须串联起来
    confirm_body = session.calls[3][2]
    assert confirm_body["init_checksum"] == "init-cs-token"
    assert confirm_body["payment_method"] == "pm_paypal_99"
...
def test_proto_stage_stripe_checkout_writes_cs_id_into_checkout_context():
    """stripe_checkout 成功路径必须把 cs_id / pm_id 等写进 checkout_context，
    供 stripe_poll 等下游 stage 复用。"""
    init_resp = {"id": "ppage_1", "init_checksum": "init-cs-token", "config_id": "cfg-7"}
    tax_resp = {"id": "ppage_1"}
    pm_resp = {"id": "pm_paypal_99"}
    confirm_resp = {
        "setup_intent": {
            "next_action": {
                "redirect_to_url": {
    ...
    session = _StubStripeSession(
        [_StubResponse(init_resp), _StubResponse(tax_resp), _StubResponse(pm_resp), _StubResponse(confirm_resp)]
    )
    ...
    assert state.checkout_context["payment_method_id"] == "pm_paypal_99"
